...
Board::Board(std::string fileName)
{
	...
	for (int y = m_height - 1; y >= 0; y--) {
		for (int x = 0; x < m_width; x++) {
			readFile >> tile;
			if (tile[0] == char('n')) {
				m_tileArr[x][y] = std::shared_ptr<Tile>(new TileNeutral());
			}
			else if (tile[0] == char('b')) {
				m_tileArr[x][y] = std::shared_ptr<Tile>(new TileBlock());
			}
				...
				m_tileArr[x][y] = std::shared_ptr<Tile>(new TilePortal(Point(xInt, yInt)));
			}
			else if (tile[0] == char('m')) {
				std::string type = tile.substr(1);
				if (type == "tl")
					m_tileArr[x][y] = std::shared_ptr<Tile>(new TileMirror(MirrorType::tl));
				else if (type == "tr")
					m_tileArr[x][y] = std::shared_ptr<Tile>(new TileMirror(MirrorType::tr));
				else if (type == "bl")
					m_tileArr[x][y] = std::shared_ptr<Tile>(new TileMirror(MirrorType::bl));
				else
					m_tileArr[x][y] = std::shared_ptr<Tile>(new TileMirror(MirrorType::br));
			}
				...
				m_tileArr[x][y] = std::shared_ptr<Tile>(new TileStart(pathId));
			}
			else if (tile[0] == char('e')) {
				int pathId = std::stoi(tile.substr(1));
				m_tileArr[x][y] = std::shared_ptr<Tile>(new TileEnd(pathId));
			}
		}
	}
